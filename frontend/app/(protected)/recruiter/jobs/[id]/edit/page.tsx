...
export default function EditJobPage(): JSX.Element {
  ...
  const query = useQuery({ queryKey: ["recruiter-job-edit", id], queryFn: () => jobsApi.byId(id) });
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) => recruiterApi.updateJob(id, parseJobPayload(values)),
    onSuccess: () => {
      pushToast({ type: "success", message: "Job updated" });
  ...
  return (
    <ProtectedRoute allowedRoles={["recruiter"]}>
      <Card>
        <h1 className="text-2xl font-bold text-brand-900">Edit Job</h1>
        <p className="mt-1 text-sm text-slate-600">
          Update this post using team templates and suggestions for better hiring quality.
        </p>
        <div className="mt-4 rounded-lg border border-brand-100 bg-brand-50 p-4">
          ...
          <p className="mt-3 text-xs text-slate-600">
            Tip: apply a role suggestion, then refine responsibilities and skill expectations.
          </p>
